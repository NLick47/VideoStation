        ...
        <div v-if="isEmpty">
            <div class="collections" v-for="(item, index) in collection" :key="item.name">
                ...
                <ul class="collection-ul">
                    <li class="collection-li"  v-for="item in item.item.slice(
                        ...
                        <div class="collection-li-inner">
                            <a :href="'/Video/' + item.videoId" target="_blank">
                                <div class="img-container">
                                    <img draggable="false"
                                        :src="this.$staticRoot + item.video.coverUri" 
                                        alt="">
                                </div>
                            </a>
                            <div class="inner-options">
                                <a style="width:100%" :href="'https://kotokawa-akira-mywife.site/web/Video/' + item.videoId"
                                    target="_blank">
                                    <div class="item-name" :title="item?.video.title">{{ itme?.video.title }}</div>
                                </a>
                                <div class="options btn-delete-collection" v-if="btnVisible"
                                    @click="removeCollection(item.video.title, item.video.id)">
                                    删除
                                </div>
                            </div>
                        </div>
                ...
                <div class="pagination">
                    <el-pagination large layout="prev, pager, next" :page-size="pageSize" :total="item.item?.length"
