<template>
                    ...
                    <div class="border-container">
                        <div class="img-container">
                            <a :href="'/video/' + item.id" target="_blank">
                                <img  loading="lazy" decoding="async" :src="this.$staticRoot+item.coverUri"
                                    alt="" >
                            </a>
                        </div>
                        <a :href="'/video/' + item.id" target="_blank">
    ...
    <div class="load-more" ref="load" @click="moreVodes">
        <b>加载更多</b>
    </div>
</template>
<script>
import BackTop from './BackTop.vue'
import {getMoreVideos} from '@/api/video'
export default {
    components: {
        BackTop
    },
    ...
    methods: {
        moreVodes()
        {
            ...
            {
                  this.isLoading = true;
                this.$refs.loadmore.innerText = "登录查看更多视频···";
                this.islock = false;
                return
            }
            this.paheNo += 1;
    ...
    async getFirstVideos() {
      this.isLoading = true;
      try {
        const data = await getMoreVideos(this.paheNo);
        this.videos = data;
            ...
            const data = await  getMoreVideos(this.paheNo);
                ...
                if (data &&data.length != 0) {
                   ...
                   data.forEach(element => {
                    ...
                    {
                         const existingVideo = this.videos.find(video => video.id === element.id);
                        if(!existingVideo)
                        {
